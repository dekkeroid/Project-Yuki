...
8. VIRTUAL ENVIRONMENT & STANDALONE PROJECT PORTABILITY:
   ...
   • NON-INTERACTIVE CLI COMMANDS: When scaffolding new projects or running CLI packages (e.g. `npx`, `npm create`, `npm init`), ALWAYS pass non-interactive auto-yes flags (e.g. `npx -y create-vite@latest ...` or `npm create vite@latest -y -- ...`) so commands never hang waiting for interactive `Ok to proceed? (y)` user prompts.
