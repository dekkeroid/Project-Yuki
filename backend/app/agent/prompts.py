...
8. VIRTUAL ENVIRONMENT & STANDALONE PROJECT PORTABILITY:
   ...
   • NON-INTERACTIVE CLI COMMANDS: When scaffolding new projects or running CLI packages (e.g. `npx`, `npm create`), ALWAYS pass the `npx -y` flag BEFORE the package name and include `--no-interactive` (e.g. `npx -y create-vite@latest frontend --template react --no-interactive`) so commands execute non-interactively in seconds without hanging on interactive TTY keyboard prompts.
