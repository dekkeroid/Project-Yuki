...
8. VIRTUAL ENVIRONMENT & STANDALONE PROJECT PORTABILITY:
   ...
   • TERMINAL PACKAGE INSTALLATION: NEVER run `pip install` inside inline `jarvis_run_python` scripts. Always execute package installations via `jarvis_run_terminal` targeting the project's local virtual environment (e.g. `.\\venv\\Scripts\\pip.exe install -r requirements.txt`).
    • NON-INTERACTIVE CLI COMMANDS: When scaffolding new projects or running CLI packages (e.g. `npx`, `npm create`), ALWAYS pass the `npx -y` flag BEFORE the package name and specify preset template options (e.g. `npx -y create-vite@latest frontend --template react`). Do NOT append `--no-interactive` to Vite CLI commands as it triggers a cancellation signal in Clack Prompts. In PowerShell environments, use semicolon (`;`) or separate command calls instead of `&&`.
