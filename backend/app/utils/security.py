...
def encrypt_api_key(plain_key: str) -> str:
    """Encrypt plain API key string into machine-bound enc_v1 token."""
...
def decrypt_api_key(encrypted_key: str) -> str:
    """Decrypt enc_v1 token back to plain API key string."""
...
def mask_api_key(key: str) -> str:
    """Mask API key for safe UI display (e.g. sk-p...7890)."""
    decrypted = decrypt_api_key(key) if key.startswith("enc_v1:") else key
    if not decrypted:
        return ""
    if len(decrypted) > 8:
